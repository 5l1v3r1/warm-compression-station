function [p, infos] = solve_pb_mpc(Ni, z0, mpc)
    ...
    if strcmp(mpc.solver,'ode-based') % Nous
        mpc.pb.alpha = 1e6*ones(size(mpc.pb.Gamma,1),1);
        mpc.pb.alpha_min = 1e15*ones(size(mpc.pb.under_z));
        mpc.pb.alpha_max = 1e15*ones(size(mpc.pb.over_z));
        mpc.pb.dt0 = 1;
    ...
    if strcmp(mpc.solver,'active-set') % QPOASIS
        ...
        try
            tic, [p,fval,status,iter,lambda] = qpOASES_sequence('h', mpc.pb.h,...% h
                                                                        mpc.pb.under_z, ... % lb
                                                                        mpc.pb.over_z, ... % ub
                                                                        [], ... % lbA
                                                                        mpc.pb.g(1:end,:), ... % ubA
