classdef SsatSimpleState < CoolProp.SS
  methods
    function varargout = exists(self, varargin)
      narginchk(1, 2)
      if nargin==1
        nargoutchk(0, 1)
        varargout{1} = CoolPropMATLAB_wrap(86, self);
      else
        nargoutchk(0, 0)
        CoolPropMATLAB_wrap(87, self, varargin{1});
      end
    end
    function self = SsatSimpleState(varargin)
      self@CoolProp.SS('_swigCreate');
      if nargin~=1 || ~ischar(varargin{1}) || ~strcmp(varargin{1},'_swigCreate')
        % How to get working on C side? Commented out, replaed by hack below
        %self.swigInd = CoolPropMATLAB_wrap(88, varargin{:});
        tmp = CoolPropMATLAB_wrap(88, varargin{:}); % FIXME
